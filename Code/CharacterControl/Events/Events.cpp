...
PE_IMPLEMENT_CLASS1(Event_MovePlayer_C_to_S, Event);
Event_MovePlayer_C_to_S::Event_MovePlayer_C_to_S(PE::GameContext &context)
: Networkable(context, this)
{
}

int Event_MovePlayer_C_to_S::packCreationData(char *pDataStream)
{
	int read = 0;
	read += PE::Components::StreamManager::WriteFloat32(m_timeStamp, &pDataStream[read]);
	read += PE::Components::StreamManager::WriteMatrix4x4(m_transform, &pDataStream[read]);
	return read;
}

int Event_MovePlayer_C_to_S::constructFromStream(char *pDataStream)
{
	int write = 0;
	write += PE::Components::StreamManager::ReadFloat32(&pDataStream[write], m_timeStamp);
	write += PE::Components::StreamManager::ReadMatrix4x4(&pDataStream[write], m_transform);
	return write;
}

void* Event_MovePlayer_C_to_S::FactoryConstruct(PE::GameContext& context, PE::MemoryArena arena)
{
	Event_MovePlayer_C_to_S *pEvt = new (arena) Event_MovePlayer_C_to_S(context);
	return pEvt;
}

PE_IMPLEMENT_CLASS1(Event_MovePlayer_S_to_C, Event);

Event_MovePlayer_S_to_C::Event_MovePlayer_S_to_C(PE::GameContext& context)
: Event_MovePlayer_C_to_S(context)
{
}

int Event_MovePlayer_S_to_C::packCreationData(char* pDataStream)
{
	int read = 0;
	read += Event_MovePlayer_C_to_S::packCreationData(&pDataStream[read]);
	read += PE::Components::StreamManager::WriteFloat32(m_receivedTime, &pDataStream[read]);
	read += PE::Components::StreamManager::WriteInt32(m_clientPlayerId, &pDataStream[read]);
	return read;
}

int Event_MovePlayer_S_to_C::constructFromStream(char* pDataStream)
{
	int write = 0;
	write += Event_MovePlayer_C_to_S::constructFromStream(&pDataStream[write]);
	write += PE::Components::StreamManager::ReadFloat32(&pDataStream[write], m_receivedTime);
	write += PE::Components::StreamManager::ReadInt32(&pDataStream[write], m_clientPlayerId);
	return write;
}

void* Event_MovePlayer_S_to_C::FactoryConstruct(PE::GameContext& context, PE::MemoryArena arena)
{
	Event_MovePlayer_S_to_C* pEvt = new (arena) Event_MovePlayer_S_to_C(context);
	return pEvt;
}
...
PE_IMPLEMENT_CLASS1(Event_Player_Move, Event);

PE_IMPLEMENT_CLASS1(Event_Player_Turn, Event);

PE_IMPLEMENT_CLASS1(Event_Player_Shoot, Event);
PE_IMPLEMENT_CLASS1(Event_Player_Hurt, Event);
};
};
