...
struct EventManager : public Component
{
	...
	static const int PE_EVENT_SLIDING_WINDOW = 128;
	...
	void processNotification(TransmissionRecord *pTransmittionRecord, bool delivered, int sendEvtNum);
	...
	EventReceptionData m_lastReceivedEvents[PE_EVENT_SLIDING_WINDOW];

	// Information about the events
	int m_needEvtNum;
	int m_leftoverEvents;
	bool m_allEvtProcessed;

	PE::NetworkContext *m_pNetContext;
	Threading::Mutex m_receiveEvtLock;
};
}; // namespace Components
}; // namespace PE
#endif
