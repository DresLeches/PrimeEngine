...
// Event manager debug
#define EVENT_THROTTLE_DEBUG 1
#define EVENT_QUEUE_LOOK 0
...
#if EVENT_QUEUE_LOOK
	memset(&m_lastReceivedEvents[0], 0, sizeof(m_lastReceivedEvents));
#endif
}
...
void EventManager::scheduleEvent(PE::Networkable *pNetworkableEvent, PE::Networkable *pNetworkableTarget, bool guaranteed)
{
	...
	{
		/*
		 * Attempt to drop events that are useless
		 */
		PEINFO("ATTEMPTING NETWORKING THROTTLING");

		// Check if the packet is guaranteed
		if (!guaranteed)
		{
			// Don't bother it's not a useful packet
			return;
		}

		// Attempt to drop non-guaranteed or zero events to throttle back the packets
		int i = 0;
		while (i < m_eventsToSend.size())
		{
			if (!m_eventsToSend[i].m_isGuaranteed)
			{
				m_eventsToSend.erase(m_eventsToSend.begin() + i);
				continue;
			}
			else if (m_eventsToSend[i].m_size == 0)
			{
				m_eventsToSend.erase(m_eventsToSend.begin() + i);
				continue;
			}
	...
	#if EVENT_THROTTLE_DEBUG
		if (haveEventsToSend() >= PE_MAX_EVENT_JAM)
		{
			printf("Events to send: %d\n", m_eventsToSend.size());
			assert(!"Still had too many events to send");
		}
	#endif
...
void EventManager::processNotification(TransmissionRecord *pTransmittionRecord, bool delivered, int sendEvtNum)
{
	for (unsigned int i = 0; i < pTransmittionRecord->m_sentEvents.size(); ++i)
	{
		...
		if (evt.m_isGuaranteed)
		{
			...
			else if (evt.m_orderId == sendEvtNum)
			{
...
int EventManager::receiveNextPacket(char *pDataStream)
{
	...
	for (int i = 0; i < numEvents; ++i)
	{
		...
		if (evtOrderId > 0)
		{
			...
			if (indexInEventsArray < 0)
			{
				...
				assert(!"Received event order that precedes current sliding window");
	...
	for (int indexInEventsArray = 0; indexInEventsArray < PE_EVENT_SLIDING_WINDOW; indexInEventsArray++)
	{
		...
		else if (numEvents != numProcessed)
		{
			m_allEvtProcessed = false;
			m_needEvtNum = indexInEventsArray + m_receiverFirstEvtOrderId;
			break;
		}
		else
		{
