...
void StreamManager::sendNextPackets()
{
	while (true)
	{
		...
		if (numEvents || numGhosts) //todo: other managers
		{
			...
			// Add the acknowledgement bit to the packet when you do UDP
			PrimitiveTypes::Bool ackBit = false;
			size += StreamManager::WriteBool(ackBit, &pPacket->m_data[size]);
...
void StreamManager::sendAckPacket()
{
	// Allocate the packet and fill it with useful data
	PE::Packet *pPacket = (PE::Packet*)(pemalloc(m_arena, PE_PACKET_TOTAL_SIZE));
	...
	// Add the acknowledgement bit to the packet when you do UDP
	bool delievered = m_pNetContext->getEventManager()->m_allEvtProcessed;
	int needEvtNum = m_pNetContext->getEventManager()->m_needEvtNum;
	PrimitiveTypes::Bool ackBit = true;

	// This should have not happen
	if (!delievered && needEvtNum == -1)
	{
		assert(!"Delievered was false, but the event that needs to be resent is -1");
	}

	size += StreamManager::WriteBool(ackBit, &pPacket->m_data[size]);
	size += StreamManager::WriteInt16(needEvtNum, &pPacket->m_data[size]);
	size += StreamManager::WriteBool(delievered, &pPacket->m_data[size]);

    // Write in the size of the packet into here
    StreamManager::WriteInt32(size, &pPacket->m_data[0]);

	// Don't need the transmission record when we start using UDP
    m_pNetContext->getConnectionManager()->sendPacket(pPacket, nullptr);

	pefree(m_arena, pPacket);
}

void StreamManager::receiveAckPacket(char* pDataStream, int& read)
{
	PrimitiveTypes::Int16 pNeedEvtNum = 0;
	bool pDelievered = false;

	read += StreamManager::ReadInt16(&pDataStream[read], pNeedEvtNum);
	read += StreamManager::ReadBool(&pDataStream[read], pDelievered);
	processNotification(pDelievered, pNeedEvtNum);
}

void StreamManager::processNotification(bool delivered, int needEvtNum)
{
	// Clean up any record that's useless
	while (m_transmissionRecords.front().m_sentEvents.size() == 0)
	{
		m_transmissionRecords.pop_front();
	}
	
	TransmissionRecord &record = m_transmissionRecords.front();
	m_pNetContext->getEventManager()->processNotification(&record, delivered, needEvtNum);
...
void StreamManager::receivePacket(Packet* pPacket)
{
	...
	// See if it is an ack packet or not
	PrimitiveTypes::Bool ackBit = false;
	read += StreamManager::ReadBool(&pPacket->m_data[read], ackBit);
	
	if (ackBit)
	{
		receiveAckPacket(pPacket->m_data, read);
	}
	else
	{
		...
		// Send the ack packets letting the server-client talk to each other
		sendAckPacket();
	}
...
static void swapEndian16(void *val)
{
...
static void swapEndian32(void *val)
{
...
int StreamManager::WriteInt16(PrimitiveTypes::Int16 v, char *pDataStream)
{
	#if !NET_LITTLE_ENDIAN
		swapEndian16(&v);
	#endif

	memcpy(pDataStream, &v, sizeof(PrimitiveTypes::Int16));
	return sizeof(PrimitiveTypes::Int16);
}

int StreamManager::ReadInt16(char *pDataStream, PrimitiveTypes::Int16 &out_v)
{
	PrimitiveTypes::Int16 v;
	memcpy(&v, pDataStream, sizeof(PrimitiveTypes::Int16));
