...
void runDrawThreadSingleFrame(PE::GameContext &ctx)
{
    ...
    {
		// Z pre-pass
		EffectManager::Instance()->setTextureAndDepthTextureRenderTargetForZPrePass();
		...
		// Clear the voxel grid to support dynamic GI (only remove invalid voxels)
		//EffectManager::Instance()->clearVoxelGrid();

		// Voxelize the scene (setup + injection)
		EffectManager::Instance()->setupVoxelization();
		EffectManager::Instance()->voxelize();

		// Perform cone tracing and store irradiance in SH grid
		EffectManager::Instance()->coneTrace();

		// Perform SSAO
		EffectManager::Instance()->ssao();

		// Compute indirect diffuse lighting by sampling the SH grid
		EffectManager::Instance()->indirectDiffuseLighting();

		// Compute indirect specular lighting by ray tracing through the voxel grids
		EffectManager::Instance()->indirectSpecularLighting();

		// Perform volumetric lighting when direct and indirect lighting are done
		EffectManager::Instance()->volumetricLighting();

		// Render the skybox
		EffectManager::Instance()->skyBoxRender();

		// Perform anti-aliasing to remove jagged edges
		EffectManager::Instance()->fxaa();

		// Convert the HDR image to LDR
		EffectManager::Instance()->toneMap();

		// Color grade the LDR image using LUT
		EffectManager::Instance()->colorGrade();

		// Perform the Motion Blur
		EffectManager::Instance()->motionBlur();

		// Perform the HUD rendering
		EffectManager::Instance()->hudRender();
