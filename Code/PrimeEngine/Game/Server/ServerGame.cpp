...
int ServerGame::runGameFrame()
{
	...
	*/
	...
	// Frame Start ---------------------------------------------------------
	float serverGameTimeBetweenFrames = m_pTimer->TickAndGetTimeDeltaInSeconds();

	// Create the Physics Events
	{
		Handle hPhysicsStartEvent("EVENT", sizeof(Event_PHYSICS_START));
		Event_PHYSICS_START* physicsStartEvent = new(hPhysicsStartEvent) Event_PHYSICS_START;

		physicsStartEvent->m_frameTime = m_frameTime;
		Events::EventQueueManager::Instance()->add(hPhysicsStartEvent, Events::QTS_GENERAL);
	}

	// Create the Update Events
	{
		Handle hUpdateEvent("EVENT", sizeof(Event_UPDATE));
		Event_UPDATE* updateEvent = new(hUpdateEvent) Event_UPDATE;

		updateEvent->m_frameTime = m_frameTime;
		Events::EventQueueManager::Instance()->add(hUpdateEvent, Events::QTS_GENERAL);
	}

	// Process the general events
	Handle eqh = Events::EventQueueManager::Instance()->getEventQueueHandle("serverGeneral");
	while (!eqh.getObject<Events::EventQueue>()->empty())
	{
		Events::Event *pGeneralEvent = eqh.getObject<Events::EventQueue>()->getFront();

		if (Event_UPDATE::GetClassId() == pGeneralEvent->getClassId())
		{
			// Update the game objects
			m_pContext->getGameObjectManager()->handleEvent(pGeneralEvent);
		}
		else if (Event_PHYSICS_START::GetClassId() == pGeneralEvent->getClassId())
		{
			// physics kick off
		}
		else if (Event_PHYSICS_END::GetClassId() == pGeneralEvent->getClassId())
		{
			// fetch the physic result
		}
		else
		{
			// Unknown event occurred
			// Let game object manager handle it
			m_pContext->getGameObjectManager()->handleEvent(pGeneralEvent);
		}
		// Even of event handling
		//PEINFO("PE_SERVER: PROGRESS: eqh destroyed event\n");
		eqh.getObject<Events::EventQueue>()->destroyFront();
	}

	// Destroy the events that were handled in this frame
	eqh.getObject<Events::EventQueue>()->destroy();

	// Update the server event time
	float serverPostEventHandleFrame = m_pTimer->TickAndGetTimeDeltaInSeconds();
	m_frameTime = serverGameTimeBetweenFrames + serverPostEventHandleFrame;
	...
	bool& gameStarted = CharacterControl::Components::ServerGameObjectManagerAddon::m_gameStarted;
	if (gameStarted)
	{
		CharacterControl::Components::ServerGameObjectManagerAddon::m_officialGameTime += m_frameTime;
	}
