...
int ClientGame::runGameFrame()
{
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				//UI
				{
					...
					//DebugRenderer::Instance()->createTextMesh("|", true, false, false, false, 0,
					//	Vector3(0.495f, 0.5f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					//DebugRenderer::Instance()->createTextMesh("--", true, false, false, false, 0,
					//	Vector3(0.4899f ,0.5f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
        ...
        else if (Event_ROTATE_CAMERA::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			Quaternion QN(pcam->m_base.getN());
			
			if (((QN.getY()) < 0.43001) && (QN.getY() > (-0.43001)))
			{
				Matrix4x4 nCan = pcam->m_base;
				nCan.turnUp(pRealEvent->m_relativeRotate.getY());
					
				Quaternion nN(nCan.getN());
				if ((((nN.getY()) > 0.43001) || (nN.getY() < (-0.43001))))
				{
						//OutputDebugStringA("not crazzy !!!\n");
						//do not change up down will break threshold
				}
				else 
				{
