...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (PrimitiveTypes::UInt32 iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
            MeshInstance *pInst = pMeshCaller->m_instances[iInst].getObject<MeshInstance>();
			pInst->m_culledOut = !pInst->isEnabled();// false; //NEW - don't draw disabled meshes
			...
			if (pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>() != nullptr)
			{
				...
				boundPoints[0] = Vector3(pMesh->m_AABB.minX, pMesh->m_AABB.minY, pMesh->m_AABB.minZ);
				boundPoints[1] = Vector3(pMesh->m_AABB.maxX, pMesh->m_AABB.maxY, pMesh->m_AABB.maxZ);
				boundPoints[2] = Vector3(boundPoints[0].getX(), boundPoints[0].getY(), boundPoints[1].getZ());
				boundPoints[3] = Vector3(boundPoints[0].getX(), boundPoints[1].getY(), boundPoints[0].getZ());
				boundPoints[4] = Vector3(boundPoints[1].getX(), boundPoints[0].getY(), boundPoints[0].getZ());
				boundPoints[5] = Vector3(boundPoints[0].getX(), boundPoints[1].getY(), boundPoints[1].getZ());
				boundPoints[6] = Vector3(boundPoints[1].getX(), boundPoints[0].getY(), boundPoints[1].getZ());
				boundPoints[7] = Vector3(boundPoints[1].getX(), boundPoints[1].getY(), boundPoints[0].getZ());

				boundPoints[0] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[0];
				boundPoints[1] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[1];
				boundPoints[2] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[2];
				boundPoints[3] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[3];
				boundPoints[4] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[4];
				boundPoints[5] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[5];
				boundPoints[6] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[6];
				boundPoints[7] = pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform * boundPoints[7];

				pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_AABB = pMesh->m_AABB;
				
				/*DebugRenderer::Instance()->createAABBMesh(false,
					pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_base,
					pInst->getFirstParentByType<PE::Components::SceneNode>().getObject<SceneNode>()->m_worldTransform,
					boundPoints);*/
			}
        }
    }
	...
	/*for (int i = 0; i < RootSceneNode::Instance()->m_playerSNs.size(); i++) {
		SceneNode* sn = RootSceneNode::Instance()->m_playerSNs[i];

		Vector3 boundPoints[8];
		boundPoints[0] = Vector3(sn->m_AABB.minX, sn->m_AABB.minY, sn->m_AABB.minZ);
		boundPoints[1] = Vector3(sn->m_AABB.maxX, sn->m_AABB.maxY, sn->m_AABB.maxZ);
		boundPoints[2] = Vector3(boundPoints[0].getX(), boundPoints[0].getY(), boundPoints[1].getZ());
		boundPoints[3] = Vector3(boundPoints[0].getX(), boundPoints[1].getY(), boundPoints[0].getZ());
		boundPoints[4] = Vector3(boundPoints[1].getX(), boundPoints[0].getY(), boundPoints[0].getZ());
		boundPoints[5] = Vector3(boundPoints[0].getX(), boundPoints[1].getY(), boundPoints[1].getZ());
		...
		boundPoints[0] = sn->m_worldTransform * boundPoints[0];
		boundPoints[1] = sn->m_worldTransform * boundPoints[1];
		boundPoints[2] = sn->m_worldTransform * boundPoints[2];
		boundPoints[3] = sn->m_worldTransform * boundPoints[3];
		boundPoints[4] = sn->m_worldTransform * boundPoints[4];
		boundPoints[5] = sn->m_worldTransform * boundPoints[5];
		boundPoints[6] = sn->m_worldTransform * boundPoints[6];
		boundPoints[7] = sn->m_worldTransform * boundPoints[7];
		...
		Vector3 rayOrigin = sn->m_base.getPos() + Vector3(0.0f, 1.5f, 0.0f);
		Vector3 rayDir = sn->m_base.getN();
		rayDir.normalize();
		Vector3 line1pts[] = {rayOrigin, color, 1000.0f* rayDir, color };
		DebugRenderer::Instance()->createLineMesh(false, sn->m_base, &line1pts[0].m_x, 2, 0);

		Vector3 color(0.0f, 1.0f, 0.0f);
		Vector3 p1 = sn->m_base.getPos() + Vector3(-0.35f, 1.0f, 0.0f);
		Vector3 p2 = sn->m_base.getPos() + Vector3(0.35f, 1.0f, 0.0f);
		Vector3 p3 = sn->m_base.getPos() + Vector3(0.0f, 1.0f, 0.35f);
		Vector3 p4 = sn->m_base.getPos() + Vector3(0.0f, 1.0f, -0.35f);
		Vector3 line1pts[] = { p1, color, p2, color, p3, color, p4, color };
		DebugRenderer::Instance()->createLineMesh(false, sn->m_base, &line1pts[0].m_x, 4, 0);
		...
		Events::Event_GATHER_DRAWCALLS *pDrawEvent, Events::Event_GATHER_DRAWCALLS_Z_ONLY *pZOnlyDrawEvent)
{
	...
	for (PrimitiveTypes::UInt32 iEffect = 0; iEffect < pEffectsForRange->m_size; ++iEffect)
	{
		...
		for (int iRenderGroup = 0; iRenderGroup < numRenderGroups; ++iRenderGroup)
		{
            ...
            PrimitiveTypes::UInt32 iSrcInstanceInBoneSegment = 0;
			for (PrimitiveTypes::UInt32 _iBoneSegment = 0; _iBoneSegment < numJointSegments; _iBoneSegment++)
			{
				...
				if (haveInstancesAndInstanceEffect) // instanced version
				{
					if (hasJointSegments)
					{
                        // TODO: WHat is this suppsoed to be?
...
void SingleHandler_DRAW::addSAs_InstancedAnimationCSMap(PE::Components::DrawList *pDrawList, PE::Components::MeshInstance *pMeshInstance,  PE::Components::Mesh *pMeshCaller, int numInstancesInGroup, int indexInInstanceList)
{
    ...
    PrimitiveTypes::UInt32 instanceIndexInTotal = indexInInstanceList;
