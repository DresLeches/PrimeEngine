...
void DebugRenderer::createAABBMesh(bool hasTransform, const Matrix4x4 &transform, const Matrix4x4 &worldMatrix,
	Vector3 * boundPoints, float scale /* = 1.0f*/)
{
	Vector3 color(0.0f, 1.0f, 0.0f);

	// rectangular cuboid
	 // top of rectangular cuboid (6-2-8-4)
	Vector3 line1pts[] = { boundPoints[5], color, boundPoints[1], color };
	createLineMesh(hasTransform, transform, &line1pts[0].m_x, 2, 0);

	Vector3 line2pts[] = { boundPoints[1], color, boundPoints[7], color };
	createLineMesh(hasTransform, transform, &line2pts[0].m_x, 2, 0);

	Vector3 line3pts[] = { boundPoints[7], color, boundPoints[3], color };
	createLineMesh(hasTransform, transform, &line3pts[0].m_x, 2, 0);

	Vector3 line4pts[] = { boundPoints[3], color, boundPoints[5], color };
	createLineMesh(hasTransform, transform, &line4pts[0].m_x, 2, 0);

	// bottom of rectangular cuboid (3-7-5-1)
	Vector3 line5pts[] = { boundPoints[2], color, boundPoints[6], color };
	createLineMesh(hasTransform, transform, &line5pts[0].m_x, 2, 0);

	Vector3 line6pts[] = { boundPoints[6], color, boundPoints[4], color };
	createLineMesh(hasTransform, transform, &line6pts[0].m_x, 2, 0);

	Vector3 line7pts[] = { boundPoints[4], color, boundPoints[0], color };
	createLineMesh(hasTransform, transform, &line7pts[0].m_x, 2, 0);

	Vector3 line8pts[] = { boundPoints[0], color, boundPoints[2], color };
	createLineMesh(hasTransform, transform, &line8pts[0].m_x, 2, 0);

	// legs (6-3, 2-7, 8-5, 4-1)
	Vector3 line9pts[] = { boundPoints[5], color, boundPoints[2], color };
	createLineMesh(hasTransform, transform, &line9pts[0].m_x, 2, 0);

	Vector3 line10pts[] = { boundPoints[1], color, boundPoints[6], color };
	createLineMesh(hasTransform, transform, &line10pts[0].m_x, 2, 0);

	Vector3 line11pts[] = { boundPoints[7], color, boundPoints[4], color };
	createLineMesh(hasTransform, transform, &line11pts[0].m_x, 2, 0);

	Vector3 line12pts[] = { boundPoints[3], color, boundPoints[0], color };
	createLineMesh(hasTransform, transform, &line12pts[0].m_x, 2, 0);
}
