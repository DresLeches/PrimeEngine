	...
	int &threadOwnershipMask)
{
    ...
    PrimitiveTypes::UInt32 numElements = pSkel->m_jointParents.m_size;
    PrimitiveTypes::UInt32 byteSize = sizeof(JointParentData) * numElements;
	...
	byteSize = sizeof(Matrix4x4) * pSkel->m_numJoints;
...
int Skeleton::l_GetSkeleton(lua_State *luaVM)
{
	...
	{
        PrimitiveTypes::UInt32 iAnim = 0;
		for ( PrimitiveTypes::UInt32 iAnimSet = 0; iAnimSet < pSkelInstance->m_hAnimationSetGPUs.m_size; ++iAnimSet)
		{
			...
			for ( PrimitiveTypes::UInt32 i = 0; i < pAnims->m_animations.m_size; i++)
			{
