...
EffectManager::EffectManager(PE::GameContext &context, PE::MemoryArena arena)
	...
	, m_LUTTextureGPU(context, arena)
	, m_SkyboxCubemapTextureGPU(context, arena)
	, m_HUDTextureGPU(context, arena)
	, m_DitherTextureGPU(context, arena)
	, m_NoiseTextureGPU(context, arena)
{
...
void EffectManager::buildFullScreenBoard()
{
	...
	m_hVoxelClearEffect = getEffectHandle("VoxelClear_CS_Tech");
	m_hVoxelInjectionSetupEffect = getEffectHandle("VoxelInjectionSetup_CS_Tech");
	m_hVoxelInjectionEffect = getEffectHandle("VoxelInjection_CS_Tech");
	m_hConeTracingEffect = getEffectHandle("SHConeTracing_CS_Tech");
	m_hSSAOEffect = getEffectHandle("SSAO_CS_Tech");
	m_hIndirectDiffuseLightingEffect = getEffectHandle("IndirectDiffuseLighting_CS_Tech");
	m_hIndirectSpecularLightingEffect = getEffectHandle("IndirectSpecularLighting_CS_Tech");
	m_hVolumetricLightingEffect = getEffectHandle("VolumetricLighting_CS_Tech");
	m_hFXAAEffect = getEffectHandle("FXAA_CS_Tech");
	m_hToneMappingEffect = getEffectHandle("ToneMapping_CS_Tech");
	m_hColorGradingEffect = getEffectHandle("ColorGrading_CS_Tech");
	m_hSkyBoxRenderEffect = getEffectHandle("SkyBoxRender_CS_Tech");
	m_hMotionBlurCSEffect = getEffectHandle("MotionBlur_CS_Tech");
	m_hHUDRenderEffect = getEffectHandle("HUDRender_CS_Tech");
}
...
void EffectManager::setTextureAndDepthTextureRenderTargetForZPrePass()
{
	m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithDepthOnly(m_hGlowTargetTextureGPU.getObject<TextureGPU>(), m_hGlowTargetTextureGPU.getObject<TextureGPU>(), true, true);
...
{
	Effect &curEffect = *m_hVoxelClearEffect.getObject<Effect>();
	...
	SamplerState &ss = SamplerStateManager::getInstance()->getSamplerState(SamplerState_MipLerp_MinTexelLerp_MagTexelLerp_Clamp);

	m_pContext->getGPUScreen()->ClearVoxelGrid(&ss);
}

void EffectManager::setupVoxelization()
{
	Effect &curEffect = *m_hVoxelInjectionSetupEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->SetupVoxelization(&ss);
}

void EffectManager::voxelize()
{
	Effect &curEffect = *m_hVoxelInjectionEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->Voxelize(&ss);
}

void EffectManager::coneTrace()
{
	Effect &curEffect = *m_hConeTracingEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->ConeTrace(&ss);
}

void EffectManager::ssao()
{
	Effect &curEffect = *m_hSSAOEffect.getObject<Effect>();
	...
	D3D11Renderer* rendererRef = (D3D11Renderer *)(m_pContext->getGPUScreen());
	rendererRef->m_pD3DContext->CSSetShaderResources(2, 1, &(m_NoiseTextureGPU.m_pShaderResourceView));

	m_pContext->getGPUScreen()->SSAO(&ss);
}

void EffectManager::indirectDiffuseLighting()
{
	Effect &curEffect = *m_hIndirectDiffuseLightingEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->IndirectDiffuseLighting(&ss);
}

void EffectManager::indirectSpecularLighting()
{
	Effect &curEffect = *m_hIndirectSpecularLightingEffect.getObject<Effect>();
	...
	D3D11Renderer* rendererRef = (D3D11Renderer *)(m_pContext->getGPUScreen());
	rendererRef->m_pD3DContext->CSSetShaderResources(5, 6, &(m_SkyboxCubemapTextureGPU.m_pShaderResourceView));

	m_pContext->getGPUScreen()->IndirectSpecularLighting(&ss);
}

void EffectManager::volumetricLighting()
{
	Effect &curEffect = *m_hVolumetricLightingEffect.getObject<Effect>();
	...
	D3D11Renderer* rendererRef = (D3D11Renderer *)(m_pContext->getGPUScreen());
	rendererRef->m_pD3DContext->CSSetShaderResources(3, 1, &(m_DitherTextureGPU.m_pShaderResourceView));
	rendererRef->m_pD3DContext->CSSetShaderResources(4, 1, &(m_shadowMapDepthTexture.m_pDepthShaderResourceView));

	m_pContext->getGPUScreen()->VolumetricLighting(&ss);
}

void EffectManager::fxaa()
{
	Effect &curEffect = *m_hFXAAEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->FXAA(&ss);
}

void EffectManager::toneMap()
{
	Effect &curEffect = *m_hToneMappingEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->ToneMap(&ss);
}

void EffectManager::colorGrade()
{
	Effect &curEffect = *m_hColorGradingEffect.getObject<Effect>();
	...
	D3D11Renderer* rendererRef = (D3D11Renderer *)(m_pContext->getGPUScreen());
	rendererRef->m_pD3DContext->CSSetShaderResources(1, 1, &(m_LUTTextureGPU.m_pShaderResourceView));

	m_pContext->getGPUScreen()->ColorGrade(&ss);
}

void EffectManager::skyBoxRender()
{
	Effect &curEffect = *m_hSkyBoxRenderEffect.getObject<Effect>();
	...
	D3D11Renderer* rendererRef = (D3D11Renderer *)(m_pContext->getGPUScreen());
	rendererRef->m_pD3DContext->CSSetShaderResources(2, 6, &(m_SkyboxCubemapTextureGPU.m_pShaderResourceView));

	m_pContext->getGPUScreen()->SkyBoxRender(&ss);
}

void EffectManager::motionBlur()
{
	Effect &curEffect = *m_hMotionBlurCSEffect.getObject<Effect>();
	...
	m_pContext->getGPUScreen()->MotionBlur(&ss);
}

void EffectManager::hudRender()
{
	Effect &curEffect = *m_hHUDRenderEffect.getObject<Effect>();
	...
	D3D11Renderer* rendererRef = (D3D11Renderer *)(m_pContext->getGPUScreen());
	rendererRef->m_pD3DContext->CSSetShaderResources(1, 1, &(m_HUDTextureGPU.m_pShaderResourceView));

	m_pContext->getGPUScreen()->HUDRender(&ss);
}
...
void EffectManager::drawSecondGlowPass()
{
	...
	TextureGPU *t = m_hGlowTargetTextureGPU.getObject<TextureGPU>();
	PE::SA_Bind_Resource setBlurTextureAction(*m_pContext, m_arena, DIFFUSE_BLUR_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture));
	setBlurTextureAction.bindToPipeline(&curEffect);

	PE::SA_Bind_Resource setTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT,
		m_2ndGlowTargetTextureGPU.m_samplerState,
		API_CHOOSE_DX11_DX9_OGL(rendererRef->m_pSRVHUDRender,
		m_2ndGlowTargetTextureGPU.m_pTexture, m_2ndGlowTargetTextureGPU.m_texture));
	setTextureAction.bindToPipeline(&curEffect);
