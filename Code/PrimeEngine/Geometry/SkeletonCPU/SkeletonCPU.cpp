	...
	Array<Matrix4x4> &res, PrimitiveTypes::UInt32 &curIndex, const Matrix4x4 &parentTransform)
{
	...
	PrimitiveTypes::UInt32 toBlendPartial[32]; // indices of slots needed to blend
	PrimitiveTypes::UInt32 nBlendPartial = 0;
	...
	*/
	...
	for (PrimitiveTypes::UInt32 iSlot = 0; iSlot < slots.m_size; iSlot++)
	{
		...
		{
			if (!(curSlot.m_flags & PARTIAL_BODY_ANIMATION)) //no partial body animation overlaps here so don't apply them
			{
				toBlendPartial[nBlendPArtial++] = iSlot;
			}
		}
		*/

		//add to either the full or partial list
		if (curSlot.m_flags & PARTIAL_BODY_ANIMATION)
		{
			if (curSlot.m_startJoint <= curIndex && curIndex <= curSlot.m_endJoint)
			{
	...
	float totalWeightPartial = 0;
	TSQ partialAvg = TSQ();

	if (nBlendPartial > 0) //if this joint has animations
	{
		//TSQ finalAvg = TSQ();
		//float totalWeight = 0;

		for (int i = 0; i < nBlendPartial; i++)
		{
			AnimationSlot &slot = slots[toBlendPartial[i]];
			// create matrix from stored TSQ
			AnimSetBufferGPU* pAnimSetGPU = hAnimSetGPUs[slot.m_animationSetIndex].getObject<AnimSetBufferGPU>();
			AnimationSetCPU* pAnimSetCPU = pAnimSetGPU->m_hAnimationSetCPU.getObject<AnimationSetCPU>();

			AnimationCPU &anim = pAnimSetCPU->m_animations[slot.m_animationIndex];
			TSQ a = anim.m_frames[slot.m_iFrameIndex0][jnt.m_index];
			TSQ b = anim.m_frames[slot.m_iFrameIndex1][jnt.m_index];
			TSQ animAvg = TSQ::SLERP(a, b, slot.m_blendFactor); //interpolate keyframes

			totalWeightPartial += slot.m_weight;
			partialAvg = TSQ::SLERP(partialAvg, animAvg, slot.m_weight / totalWeightPartial); //blend based on animation weight
		}
	}

	//blend all the animations together
	if (nBlend > 0 && totalWeightPartial < 1.0f) //if this joint has animations, and the partial body layer isn't at full strength
	{
		TSQ finalAvg = TSQ();
		float totalWeight = 0;
		...
		{
			AnimationSlot &slot = slots[toBlend[i]];
			// create matrix from stored TSQ
			AnimSetBufferGPU* pAnimSetGPU = hAnimSetGPUs[slot.m_animationSetIndex].getObject<AnimSetBufferGPU>();
			AnimationSetCPU* pAnimSetCPU = pAnimSetGPU->m_hAnimationSetCPU.getObject<AnimationSetCPU>();
			...
			totalWeight += slot.m_weight;
			finalAvg = TSQ::SLERP(finalAvg, animAvg, slot.m_weight / totalWeight); //blend based on animation weight
		}

		partialAvg = TSQ::SLERP(finalAvg, partialAvg, totalWeightPartial); //apply the partial body layer as an overlay
	}

	local = partialAvg.createMatrix(); //create the matrix from the final result
