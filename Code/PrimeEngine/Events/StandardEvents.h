...
struct Event_CLIENT_SERVER_CONNECTED_ACK : public Event, public Networkable {
	...
	PrimitiveTypes::Bool m_connected;
};

// Event server client player joined
struct Event_SERVER_CLIENT_PLAYER_JOINED : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_SERVER_CLIENT_PLAYER_JOINED);
	PE_DECLARE_NETWORKABLE_CLASS

	Event_SERVER_CLIENT_PLAYER_JOINED(PE::GameContext &context);
	virtual ~Event_SERVER_CLIENT_PLAYER_JOINED() {}
	...
	PrimitiveTypes::Int32 m_newPlayerId; // id given to client by server
};

// General packet receive acknowledgement for both server and client
struct Event_NETWORK_PACKET_ACK : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_NETWORK_PACKET_ACK);
	PE_DECLARE_NETWORKABLE_CLASS;

	Event_NETWORK_PACKET_ACK(PE::GameContext &context);
	virtual ~Event_NETWORK_PACKET_ACK() {}
	...
	PrimitiveTypes::Int16 m_evtNum; // Send notification that this event was delievered
	PrimitiveTypes::Int16 m_needEvtNum;   // Send notification that client needs this event
	PrimitiveTypes::Bool m_delivered;
};

struct Event_SERVER_CLIENT_LOAD_LEVEL : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_SERVER_CLIENT_LOAD_LEVEL);
	PE_DECLARE_NETWORKABLE_CLASS

	Event_SERVER_CLIENT_LOAD_LEVEL(PE::GameContext &context);
	virtual ~Event_SERVER_CLIENT_LOAD_LEVEL() {}
	...
	PrimitiveTypes::Int32 m_packageId;
	PrimitiveTypes::Int32 m_levelId;
	Vector3 mSpawnPos;
};

// Client network manager tells the server that client got killed in game
struct Event_CLIENT_SERVER_KILLED : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_CLIENT_SERVER_KILLED);
	PE_DECLARE_NETWORKABLE_CLASS

	Event_CLIENT_SERVER_KILLED(PE::GameContext &context);
	virtual ~Event_CLIENT_SERVER_KILLED() {}
	...
	PrimitiveTypes::Int32 m_clientId; // Who killed the player
	PrimitiveTypes::Int32 m_killedPlayerId; // Which player got killed
};

//Shooter tells server to inform enemy that they've been shot
struct Event_CLIENT_SERVER_TAKEDAMAGE : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_CLIENT_SERVER_TAKEDAMAGE);
	PE_DECLARE_NETWORKABLE_CLASS

		Event_CLIENT_SERVER_TAKEDAMAGE(PE::GameContext &context);
	virtual ~Event_CLIENT_SERVER_TAKEDAMAGE() {}
	...
	float m_damage; // damage taken
	PrimitiveTypes::Int32 m_hitPlayerId; // Which player got shot
};

// Server tells the clients to respawn
struct Event_SERVER_CLIENT_RESPAWN : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_SERVER_CLIENT_RESPAWN);
	PE_DECLARE_NETWORKABLE_CLASS;

	Event_SERVER_CLIENT_RESPAWN(PE::GameContext &context);
	virtual ~Event_SERVER_CLIENT_RESPAWN() {}
	...
	Vector3 m_position; // Tell client their position. 
						// NOTE: Using Vector4 until I finish writing the Read/Write functions in StreamManager
	PrimitiveTypes::Int32 m_clientId; // Debug: Ensure that the correct client is getting the position
};

//Server tells client to take damage
struct Event_SERVER_CLIENT_TAKEDAMAGE : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_SERVER_CLIENT_TAKEDAMAGE);
	PE_DECLARE_NETWORKABLE_CLASS;

	Event_SERVER_CLIENT_TAKEDAMAGE(PE::GameContext &context);
	virtual ~Event_SERVER_CLIENT_TAKEDAMAGE() {}
	...
	PrimitiveTypes::Int32 m_damage; // Tell client how much damage to take
	PrimitiveTypes::Int32 m_clientId; // Debug: Ensure that the correct client is getting the damage
};

// Client tells the server the kill you got
struct Event_SERVER_CLIENT_SEND_KILL_SCORE : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_SERVER_CLIENT_SEND_KILL_SCORE);
	PE_DECLARE_NETWORKABLE_CLASS;

	Event_SERVER_CLIENT_SEND_KILL_SCORE(PE::GameContext &context);
	virtual ~Event_SERVER_CLIENT_SEND_KILL_SCORE() {}
	...
	// Score
	PrimitiveTypes::Int32 m_killUpdate;  // Tell the server you got a kill
	PrimitiveTypes::Int32 m_clientId;	 // Tell which client's score to update
